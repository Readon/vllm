...
class SpecDecodeBaseProposer:
    def __init__(
        ...
        # DDTree: pre-allocate fixed-size GPU buffer for ancestor-only attention.
        # Shape: (1 + max_spec, 1 + max_spec), always the same tensor identity.
        # During DDTree verification: filled with ancestor mask.
        # During normal decode: stays as zeros (no effect on attention).
        # This ensures CUDA graph capture always includes the qq_bias path,
        # and graph replay uses the same data_ptr with updated content.
        if self.speculative_config.use_ddtree():
            ...
            self._ddtree_qq_biases = []   # per-request masks (CPU, for tree walk)
            self._ddtree_child_maps = []  # per-request child maps
        else:
        ...
        if self.uses_mrope:
            ...
            # For draft_model method with M-RoPE, also allocate a 1D positions
            # buffer as fallback for kernels that don't support M-RoPE yet.
    ...
    def propose(
        ...
        # DDTree: collect top-K log-probs at each MTP step
        ddtree_enabled = (hasattr(self, 'speculative_config')
                          and self.speculative_config.use_ddtree())
        ddtree_topk = self.speculative_config.ddtree_topk if ddtree_enabled else 0
        ddtree_top_log_probs = [] if ddtree_enabled else None
        ddtree_top_token_ids = [] if ddtree_enabled else None
        # For DDTree, limit MTP chain depth to ddtree_mtp_depth if specified
        if ddtree_enabled and self.speculative_config.ddtree_mtp_depth > 0:
            mtp_loop_limit = self.speculative_config.ddtree_mtp_depth - 1
        else:
            mtp_loop_limit = self.num_speculative_tokens - 1

        if ddtree_enabled:
            # Compute logits once for both greedy and top-K
            logits = self.model.compute_logits(sample_hidden_states)
            draft_token_ids = logits.argmax(dim=-1)
            # Extract top-K log-probs (log_softmax needed for correct
            # probability comparisons in DDTree builder)
            topk_logits, topk_ids = torch.topk(
                logits.float(), k=ddtree_topk, dim=-1)
            # Use logits as proxy for log-probs (monotonic transform,
            # sufficient for relative ordering in DDTree)
        ...
        for token_index in range(mtp_loop_limit):
            ...
            # DDTree: compute logits once for both greedy sample and top-K
            if ddtree_enabled:
        ...
        # DDTree: build tree from top-K log-probs and return flattened tree
        if ddtree_enabled and ddtree_top_log_probs is not None:
            from vllm.v1.spec_decode.ddtree import build_ddtree
            from vllm.v1.spec_decode.ddtree import ddtree_to_qq_bias
            spec_config = self.speculative_config
            # Per-request DDTree building
            all_tree_token_ids = []
            self._ddtree_child_maps = []  # store for verification
            # Reset the pre-allocated qq_bias buffer to zeros
            self._ddtree_qq_buf.zero_()
            for req_idx in range(batch_size):
                # Stack top-K across MTP steps: [L, K]
                tp = torch.stack([t[req_idx] for t in ddtree_top_log_probs],
                                 dim=0)
                ti = torch.stack([t[req_idx] for t in ddtree_top_token_ids],
                                 dim=0)
                tree = build_ddtree(
                    tp, ti,
                ...
                # Flatten tree tokens in DFS order
                tree_toks = tree.token_ids  # list[int]
                all_tree_token_ids.append(tree_toks)
                self._ddtree_child_maps.append(tree.child_maps)
                # Copy ancestor-only attention mask into pre-allocated buffer
                # For single-request case: write directly into _ddtree_qq_buf
                if req_idx == 0:
                    N = tree.total_slots
            ...
            if max_tree_len > 0:
                padded = torch.zeros(batch_size, max_tree_len,
    ...
    def validate_same_kv_cache_group(self, kv_cache_config: KVCacheConfig) -> None:
        ...
        # For draft_model method, layers may span multiple KV cache groups
        # when the draft model has a different architecture from the target.
        # Skip the single-group assertion in this case.
        if self.method != "draft_model":
            assert (
                len(
                    set(
    ...
    def initialize_attn_backend(
        ...
        # Find which kv_cache_group(s) the draft layers belong to
        self.validate_same_kv_cache_group(kv_cache_config)
        
        # Map each draft layer to its KV cache group
        ...
        # Use the first group found as the primary group ID
        kv_cache_spec = None
        for gid, group in enumerate(kv_cache_config.kv_cache_groups):
        ...
        if kv_cache_spec is not None:
            for layer_name in self._draft_attn_layer_names:
                ...
                if backend_key not in attention_groups:
                    # Get the KV cache spec and group ID for this specific layer
                    gid, group_spec = layer_to_group.get(
                        layer_name, (self.kv_cache_gid, kv_cache_spec)
                    )
